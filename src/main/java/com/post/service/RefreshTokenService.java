...
import com.post.repository.RefreshTokenRepository;
...
public class RefreshTokenService extends BaseService<RefreshToken,Long>{
    ...
    public RefreshToken createRefreshToken(Long userId) {
        ...
        if(refreshTokenRepository.findByUserId(userId).isPresent()){
            refreshTokenRepository.delete(refreshTokenRepository.findByUserId(userId).get());
        }
    ...
    public RefreshToken verifyExpiration(RefreshToken token) {
        if (token.getExpiryDate().compareTo(Instant.now()) < 0) {
            ...
            throw new UnAuthenticatedException("Refresh token "+ token.getToken() +"  was expired. Please Login again");
        }
